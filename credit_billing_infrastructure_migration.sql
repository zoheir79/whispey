...
COMMENT ON COLUMN monthly_consumption.kb_storage_cost IS 'Coût stockage KB mensuel';
COMMENT ON COLUMN monthly_consumption.kb_search_tokens IS 'Tokens utilisés pour recherches KB';
COMMENT ON COLUMN monthly_consumption.kb_search_cost IS 'Coût recherches KB';
COMMENT ON COLUMN monthly_consumption.kb_embedding_tokens IS 'Tokens utilisés pour création embeddings';
COMMENT ON COLUMN monthly_consumption.kb_embedding_cost IS 'Coût création embeddings (one-shot)';
COMMENT ON COLUMN monthly_consumption.workflow_operations IS 'Nombre d\'opérations workflow exécutées';
COMMENT ON COLUMN monthly_consumption.workflow_cost IS 'Coût total workflow pour le mois';
COMMENT ON COLUMN monthly_consumption.workflow_execution_minutes IS 'Minutes d\'exécution workflow';

-- ========================================
-- 2. TABLE GESTION DES CRÉDITS UTILISATEUR
-- ========================================
...
-- Commentaires
COMMENT ON TABLE user_credits IS 'Gestion des crédits utilisateur par workspace';
COMMENT ON COLUMN user_credits.current_balance IS 'Balance actuelle en crédits';
COMMENT ON COLUMN user_credits.credit_limit IS 'Limite maximale de crédits autorisée';
COMMENT ON COLUMN user_credits.low_balance_threshold IS 'Seuil d\'alerte balance faible';

-- ========================================
-- 3. TABLE CYCLES DE FACTURATION UNIFIÉE
-- ========================================
...
COMMENT ON COLUMN billing_cycles.fixed_cost IS 'Coûts fixes (dedicated, subscription)';
COMMENT ON COLUMN billing_cycles.pag_cost IS 'Coûts variables (pay-as-you-go)';
COMMENT ON COLUMN billing_cycles.prorated_cost IS 'Coûts proratés pour cycles partiels';

-- ========================================
-- 4. TABLE TRANSACTIONS CRÉDITS
-- ========================================
...
-- Commentaires
COMMENT ON TABLE credit_transactions IS 'Historique des transactions de crédits utilisateur';
COMMENT ON COLUMN credit_transactions.transaction_type IS 'Type: deduction (négatif), recharge/refund (positif)';
COMMENT ON COLUMN credit_transactions.amount IS 'Montant: négatif pour déduction, positif pour recharge';
COMMENT ON COLUMN credit_transactions.service_type IS 'Type de service à l\'origine de la transaction';
...
-- Commentaires
COMMENT ON TABLE credit_alerts IS 'Système d\'alertes pour la gestion des crédits';
COMMENT ON COLUMN credit_alerts.alert_type IS 'Type d\'alerte: low_balance, service_suspended, etc.';
COMMENT ON COLUMN credit_alerts.severity IS 'Niveau de sévérité: info, warning, critical';
...
BEGIN
    ...
    RAISE NOTICE '✅ Migration des tables crédits et facturation réussie';
    RAISE NOTICE '✅ Tables créées: user_credits, billing_cycles, credit_transactions, credit_alerts';
    RAISE NOTICE '✅ Extensions ajoutées à monthly_consumption';
    RAISE NOTICE '✅ Indexes et contraintes appliqués';
